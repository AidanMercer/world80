...
// Centerpiece desktop widget: a white Arch logo wrapped in an audio-reactive
// triangle. We feed it the same way the old side spectrum worked — `cava` in
// raw-ascii mode (cava.conf) streams one frame per line, N values 0..1000 — and
// bend that spectrum around the three edges of a triangle outline.
//
// Bottom layer (above wallpaper, below windows) and fully click-through, so it's
// passive scenery. No blur layerrule: it's a transparent glow over the wallpaper.
PanelWindow {
    id: root
    ...
    Item {
        ...
        // GPU glow: a blurred, periwinkle-tinted copy of the crisp outline below,
        // declared first so it sits behind. The blur runs on the scene-graph (GPU),
        // not as a per-frame software gaussian, so it's cheap even while reacting.
        MultiEffect {
            source: canvas
            anchors.fill: canvas
